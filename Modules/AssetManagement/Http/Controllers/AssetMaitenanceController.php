...
class AssetMaitenanceController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            $query = AssetMaintenance::where('asset_maintenances.business_id', $business_id)
                        ->leftJoin('assets as a', 'a.id', '=', 'asset_maintenances.asset_id')
            ...
            return Datatables::of($asset_maintenances)
                ...
                ->removeColumn('id')
                ->rawColumns(['status', 'priority', 'action', 'created_at', 'assigned_to_user'])
                ->make(true);
        }
    ...
    public function store(Request $request)
    {
        ...
        try {
            $input = $request->only('status', 'priority', 'details', 'asset_id');
